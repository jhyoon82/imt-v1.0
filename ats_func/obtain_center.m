function cur_state = obtain_center(varargin)
...
corners = M * corners;
width = sqrt((corners(1,1)-(corners(1,2)))^2+(corners(2,1)-(corners(2,2)))^2);
height = sqrt((corners(1,2)-(corners(1,3)))^2+(corners(2,2)-(corners(2,3)))^2);
center = mean(corners(:,1:4),2);
cur_state = [center',width,height];
