function [tmpl] = sampling_state_v2(tmpl,opt,param, RandMatrix)
...
for i=1:opt.num_tracker
    ...
    tmpl(i).sample(:,IND_Dynamic) = tmpl(i).sample(:,IND_Dynamic) + repmat(param.translation,1,Siz_Dynamic) + diag([opt.affsig2]')*RandMatrix(:,1:Siz_Dynamic);
    Siz_RandomWalk = size(IND_RandomWalk,2);
    tmpl(i).sample(:,IND_RandomWalk) = tmpl(i).sample(:,IND_RandomWalk) + diag([opt.affsig1]')*RandMatrix(:,1:Siz_RandomWalk);
